...
export const FamilyHeader: React.FC<FamilyHeaderProps> = ({
  ...
  const currentAcademicYear = currentDate.getMonth() >= 8 ? currentDate.getFullYear() : currentDate.getFullYear() - 1;
  const academicYearOptions = [];
  for (let i = 0; i < 3; i++) {
    const year = currentAcademicYear - i;
    academicYearOptions.push(`${year}-${year + 1}`);
  }
  return <div className="bg-white border-b border-gray-200 p-3 sm:p-6 mb-4 sm:mb-6 shadow-sm">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-4">
        <div className="flex flex-col sm:flex-row sm:items-center gap-4 flex-1">
          <div className="min-w-0 flex-1">
            ...
            <select value={selectedFamily?.id || ''} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => {
          ...
          <div className="min-w-0">
            ...
            <div className="relative">
              ...
              <select value={academicYear} onChange={(e: React.ChangeEvent<HTMLSelectElement>) => onAcademicYearChange(e.target.value)} className="w-full sm:w-23 pl-10 pr-3 py-2 border border-gray-300 rounded-lg bg-white text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                {academicYearOptions.map(year => <option key={year} value={year}>
                    {year}
                  </option>)}
              </select>
            </div>
      ...
      {selectedFamily && <div className="bg-gray-50 rounded-lg p-4">
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-3">
            <div className="min-w-0">
              <h3 className="text-lg font-semibold text-gray-900 break-words">
                {selectedFamily.name}
              </h3>
              {selectedFamily.primary_contact_name && <p className="text-sm text-gray-600 break-words">
            ...
            <div className="text-left sm:text-right">
              ...
              <div className={`text-sm font-medium ${progressPercentage >= 100 ? 'text-green-600' : progressPercentage >= 75 ? 'text-yellow-600' : progressPercentage >= 50 ? 'text-blue-600' : 'text-red-600'}`}>
                {getProgressStatus()}
              </div>
            </div>
          </div>
