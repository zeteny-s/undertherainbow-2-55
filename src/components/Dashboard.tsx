...
export const Dashboard: React.FC = () => {
  ...
  // Partner normalization utilities to merge company variants (Kft., Bt., Ev., punctuation, etc.)
  const normalizePartnerName = (name: string): string => {
    if (!name) return '';
    // Trim and lowercase
    let normalized = name.trim().toLowerCase();
    // Remove diacritics for robust matching (grouping key only)
    normalized = normalized.normalize('NFD').replace(/[\u0300-\u036f]/g, '');
    // Replace common punctuation with spaces
    normalized = normalized.replace(/[.,]/g, ' ');
    // Collapse multiple spaces
    normalized = normalized.replace(/\s+/g, ' ');
    // Remove Hungarian legal form tokens (word boundary)
    const legalTokens = [
      'kft', 'bt', 'kkt', 'zrt', 'nyrt', 'ev', 'e v', 'e v', 'e.v', 'e. v',
      'egyeni vallalkozo'
    ];
    const tokensPattern = new RegExp(`\\b(${legalTokens.join('|')})\\b`, 'g');
    normalized = normalized.replace(tokensPattern, ' ');
    // Cleanup extra spaces again
  ...
  const prettyPartnerName = (name: string): string => {
    ...
    // Remove trailing legal tokens (one or two-word forms)
    const removeTrailingToken = (input: string): string => {
      ...
      // Remove single last token
      if (singleTokens.has(last)) {
        return words.slice(0, -1).join(' ');
      }
      // Remove two-word token at the end (e.g., "Egyéni Vállalkozó")
    ...
    const cleaned = removeTrailingToken(cleanedOnce); // run twice in case of punctuation variations
    return cleaned.trim();
  };
  ...
  const generateMonthlyData = (invoices: any[], yearFilter: 'all' | number = 'all', monthFilter: 'all' | number = 'all') => {
    ...
    // If a specific month is selected, return data for only that month
    if (monthFilter !== 'all') {
      const selectedMonthIndex = monthFilter - 1;
    ...
    // Show all months - default behavior
    return months.map((month, index) => {
      let monthInvoices = invoices.filter(inv => {
        ...
        // Filter by current month being processed
        if (invoiceMonth !== index) return false;
        
        // If no specific year filter, show current year by default
              ...
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2">
                ...
                {chartData.munkaszamData.length > 5 && (
                  <button
                    ...
                    className="px-3 py-1.5 text-xs sm:text-sm font-medium rounded-lg text-blue-700 bg-blue-50 hover:bg-blue-100 transition-colors whitespace-nowrap"
                  >
