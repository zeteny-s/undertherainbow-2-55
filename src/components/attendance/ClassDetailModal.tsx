  ...
  const fetchStudents = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('students').select('*').eq('class_id', classData.id).order('name');
  ...
  const fetchPedagogusProfiles = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('profiles').select('id, name, email').eq('profile_type', 'pedagogus').order('name');
      if (error) throw error;
  ...
  const fetchAttendanceStats = async () => {
    try {
      const {
        data,
        error
      } = await supabase.from('attendance_records').select('student_id, present').eq('class_id', classData.id);
      if (error) throw error;
      ...
      <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b">
          ...
          <div className="flex items-center space-x-2">
            <button onClick={() => setIsEditing(!isEditing)} className="p-2 text-gray-600 hover:bg-gray-100 rounded-lg transition-colors">
              <Edit className="h-5 w-5" />
            </button>
            <button onClick={handleDeleteClass} className="p-2 text-red-600 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors">
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="bg-gray-50 rounded-lg p-4">
            ...
            {isEditing ? <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" value={editingClass.name} onChange={e => setEditingClass(prev => ({
                ...prev,
                name: e.target.value
                 ...
                 <div>
                   ...
                   <select value={editingClass.house} onChange={e => setEditingClass(prev => ({
                ...prev,
                ...
                <div className="md:col-span-2">
                  <label className="block text-sm font-medium text-gray-700 mb-2">Pedagógusok</label>
                  <select value={editingClass.pedagogus_id} onChange={e => setEditingClass(prev => ({
                ...prev,
                pedagogus_id: e.target.value
              }))} className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500">
                ...
                <div className="md:col-span-2 flex justify-end space-x-2">
                  <button onClick={() => setIsEditing(false)} className="px-4 py-2 text-gray-700 border border-gray-300 rounded-lg hover:bg-gray-50">
                    Mégse
                  </button>
                  <button onClick={handleUpdateClass} className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700">
                    Mentés
                  </button>
                </div>
          ...
          <div>
            ...
            <div className="flex items-center space-x-2 mb-4">
              <input type="text" value={newStudentName} onChange={e => setNewStudentName(e.target.value)} placeholder="Új gyerek neve" className="flex-1 px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500" onKeyPress={e => e.key === 'Enter' && handleAddStudent()} />
              <button onClick={handleAddStudent} disabled={!newStudentName.trim()} className="flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 disabled:opacity-50">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto">
              {students.map(student => <div key={student.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <button onClick={() => handleDeleteStudent(student.id)} className="p-2 text-red-500 hover:text-red-700 hover:bg-red-50 rounded-lg transition-colors">
                    <Trash2 className="h-4 w-4" />
                  </button>
                </div>)}
            </div>

            {students.length === 0 && <div className="text-center py-8 text-gray-500">
