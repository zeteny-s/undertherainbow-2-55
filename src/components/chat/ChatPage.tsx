...
export const ChatPage: React.FC = () => {
  ...
  const fetchConversations = async () => {
    const {
      data,
      error
    } = await supabase.from('chat_conversations').select('*').order('last_message_at', {
      ascending: false
    });
  ...
  const fetchFolders = async () => {
    const {
      data,
      error
    } = await supabase.from('chat_folders').select('*').order('name');
    if (error) throw error;
    setFolders(data || []);
  };
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      await supabase.from('chat_messages').insert(fallbackResponse);
      if (conversationId) {
        fetchMessages(conversationId);
        fetchConversations();
      }
  ...
  return <div className="min-h-screen bg-gradient-to-br from-gray-50 to-gray-100 flex">
      ...
      <div className={`${sidebarOpen ? 'w-80' : 'w-0'} transition-all duration-300 ease-in-out bg-white border-r border-gray-200 flex flex-col ${isMobile ? 'fixed inset-y-0 left-0 z-50 shadow-xl' : 'relative'}`}>
        {sidebarOpen && <>
            ...
            return <div key={folder.id} className="mb-4">
              ...
              {conversations.filter(c => !c.folder_id).length > 0 && <div>
                  {folders.length > 0 && <div className="px-3 py-2 text-sm font-medium text-gray-500 border-t border-gray-200 pt-4">
                      Egyéb beszélgetések
                    </div>}
                  {conversations.filter(c => !c.folder_id).map(conversation => <button key={conversation.id} onClick={() => {
              selectConversation(conversation.id);
              if (isMobile) setSidebarOpen(false);
            }} className={`w-full text-left px-3 py-3 text-sm rounded-lg transition-colors ${activeConversation === conversation.id ? 'bg-blue-50 text-blue-700 border-l-4 border-blue-500' : 'hover:bg-gray-50 text-gray-700'}`}>
                      <div className="flex items-center space-x-2">
                        <MessageSquare className="w-4 h-4 flex-shrink-0" />
                        <span className="truncate">{conversation.title}</span>
                      </div>
              ...
              {conversations.length === 0 && <div className="text-center py-8 text-gray-500">
            ...
            <div className="p-4 border-t border-gray-200">
              <button onClick={() => setShowHistory(true)} className="w-full flex items-center space-x-2 px-3 py-2 text-sm text-gray-600 hover:bg-gray-50 rounded-lg transition-colors">
      ...
      {/* Mobile Overlay */}
      {isMobile && sidebarOpen && <div className="fixed inset-0 bg-black bg-opacity-50 z-40" onClick={() => setSidebarOpen(false)} />}
        ...
        <div className="bg-white border-b border-gray-200 px-4 py-3">
          <div className="flex items-center space-x-3">
            ...
            <div className="flex-1">
              {activeConversation ? <h1 className="text-lg font-semibold text-gray-900">
                  {conversations.find(c => c.id === activeConversation)?.title || 'Beszélgetés'}
                </h1> : <h1 className="text-lg font-semibold text-gray-900">AI Chat Asszisztens</h1>}
            </div>
          </div>
        </div>

        {/* Chat History Modal */}
        <ChatHistoryModal isOpen={showHistory} onClose={() => setShowHistory(false)} conversations={conversations} folders={folders} activeConversation={activeConversation} onSelectConversation={selectConversation} onCreateConversation={createNewConversation} onRefreshData={fetchData} />
