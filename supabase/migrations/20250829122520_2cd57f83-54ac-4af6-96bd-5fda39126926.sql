...
-- Create some sample pedagogus profiles for testing
-- First, we need to insert into auth.users (simulation - in real app this would be done via signup)
-- For now, let's create profiles that can be associated with classes

-- Insert sample pedagogus profiles (these will be orphaned profiles for demo purposes)
INSERT INTO public.profiles (id, user_id, name, profile_type) VALUES
(gen_random_uuid(), gen_random_uuid(), 'Nagy Péter', 'pedagogus'),
(gen_random_uuid(), gen_random_uuid(), 'Kovács Anna', 'pedagogus'),
(gen_random_uuid(), gen_random_uuid(), 'Szabó Katalin', 'pedagogus'),
(gen_random_uuid(), gen_random_uuid(), 'Tóth László', 'pedagogus')
