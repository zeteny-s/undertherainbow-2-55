...
CREATE TABLE public.newsletters (
  ...
  title TEXT NOT NULL,
  description TEXT,
  campus USER-DEFINED NOT NULL REFERENCES (SELECT unnest(enum_range(NULL::campus_type))),
...
-- Create a function to get forms by campus for newsletter creation
CREATE OR REPLACE FUNCTION public.get_forms_by_campus(campus_filter text DEFAULT NULL)
RETURNS TABLE (
  id uuid,
  title text,
  description text,
...
AS $$
  SELECT f.id, f.title, f.description, f.campus, f.created_at
  FROM public.forms f
  WHERE f.status = 'active'::form_status
    AND (campus_filter IS NULL OR f.campus::text = campus_filter)
